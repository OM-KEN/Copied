...
- Action 优先级：符合条件的 Lithe 图片文件独占主按钮并进入菜单；否则首个非颜色检测为主按钮，其余进菜单。没有可用的检测操作时，短文本搜索、长文本另存为；纯语言类型也保留此回退按钮。
- 内联 Action 只在 `copyText` 非空时把主按钮/快速触发改为复制；错误结果不提供复制入口。词典预查只在 `ActionResolver.makeAction()`，不得塞进 50ms 检测器；点击直接复用预查释义。
...
- `VERSION` 是构建版本单一来源。Release 资产名固定为 `Copied-<VERSION>.dmg`，tag 为 `v<VERSION>`，标题为 `Copied v<VERSION> (macOS)`；正文格式遵循 `git-push` skill，发布后用 API 核对名字、大小和 SHA-256。
